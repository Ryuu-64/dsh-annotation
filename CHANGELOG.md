...
### 新功能
- **侧边栏文件批注（上游 `6ea8783`，要求 DSH 0.1.5-rc.2）**：在工作区文件预览中选中
  文本、Markdown 或代码正文，即可沿用同一套批注、保存和回车发送流程；发给模型的引用和
  编辑卡片都会带上 `[文件路径]` 前缀。同文不同文件可分别批注，刷新并重新打开原文件后
  按文件恢复标记。当前不包含 PDF、图片和 iframe 内 HTML。
  上游该功能直接读 `sessions.list.getSnapshot().current` 判断选区所属会话，本 fork 改走
  `currentSessionId()` 的多级兜底（该字段在 DSH 0.1.6-alpha.2+ 已移除）；取不到会话时按
  「不属于当前会话」处理，宁可拒绝这次批注，也不把别的会话的文件选区挂到本会话上。

## [1.4.10-ryuu.1] - fork 首发

基线：上游 `@changfenhuang/dsh-annotation@1.4.10`（MIT）。**只改浏览器端 `client.js`
的悬浮面板（hover tip）子系统**，其余行为不变。

### 修复
- **`[A]` 悬浮面板归属模型（对齐上游 PR #65）**：三类面板（输入框旁胶囊 / 用户气泡上的
  「批注 ×N」标签 / 回复里的 `Annotation N` 芯片）共享单例容器 `tipLayer`，而上游的
  `updateChip()` 在「无待发送批注」分支里**无条件**清空它。该函数由 `scroll`(capture) /
  `resize` / body 级 `MutationObserver` 经 `onLayoutChange()` 高频调用，于是已发送消息上
  的标签/芯片一 hover 就被抹掉——表现为「鼠标悬停到批注上只显示一下，然后马上消失」。
  现引入 `tipOwner` / `clearTip(owner)` / `presentTip(owner, el)`：清空必须指名归属，
  无归属清空只保留给语言切换与会话切换两处显式场景。
- **`[B]` 跨间隙不再靠运气**：面板与触发元素之间有 6px 间隙，上游只给 250ms 固定宽限，
  鼠标慢一点面板就消失。现在宽限到点时复查**实时**指针位置（`pointermove` 跟踪，而非
  `mouseleave` 事件里的过期坐标），指针仍在「触发元素 + 面板 + 间隙容差」并集内就不关闭。
- **`[C]` 宽限定时器按归属收敛**：上游每个面板各持一个 `hide` 定时器却清同一个共享容器，
  快速从 A 滑到 B 时 A 遗留的定时器会误杀 B 刚显示的面板。现所有面板共用一个计时器句柄，
  且关闭前校验「自己仍是当前归属」。
- **`[D]` 面板监听器不再无界泄漏**：上游每次渲染气泡标签/回复芯片都往同一个 `tipLayer`
  追加 `mouseenter` / `mouseleave`，而消息重渲染会重建元素却不移除旧监听器，导致监听器
  与定时器无界累积。现改为两处固定监听器 + 按 `tipOwner` 分派。
- `client.js` 的 `exports.name` 与 ModuleLoader `id` 同步改为 `@ryuu-64/dsh-annotation`
  （必须与 `package.json` 的 `name` 严格一致，否则 client-modules 报
  `bundle loaded without registering "..."`）。

### 工程
- 包名、仓库地址、作者与归属声明改为本 fork；新增 `NOTICE.md` 记录与上游的关系及改动范围。
...
- 复核选区、保存与刷新恢复，以及空正文发送、编辑窗口拖动和高亮避开输入框的已有修复（#49、#50、#52）。
